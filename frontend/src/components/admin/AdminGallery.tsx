...
export const AdminGallery: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {images.map((image) => (
          <Card key={image._id}>
            ...
            <h3 className="text-sm font-semibold text-black mb-1 truncate">{image.title}</h3>
      ...
      {images.length === 0 && (
        <div className="text-center text-gray-500 mt-8">No images in gallery</div>
      )}
      ...
      <Modal
        ...
        title={editingImage ? 'Edit Gallery Image' : 'Upload Gallery Image'}
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          {!editingImage && (
            <div>
              <label className="block text-sm font-medium text-black dark:text-white mb-1">Upload Image</label>
              <input
                type="file"
                accept="image/*"
          ...
          <div>
            <label className="block text-sm font-medium text-black dark:text-white mb-1">Family Member ID (optional)</label>
            <input
              type="number"
              value={formData.familyMemberId || ''}
              onChange={(e) => setFormData({ ...formData, familyMemberId: e.target.value ? parseInt(e.target.value) : null })}
              className="w-full px-3 py-2 border border-black rounded-md"
            />
          </div>
          ...
          {editingImage && (
            <div>
              <label className="block text-sm font-medium text-black dark:text-white mb-1">Image URL</label>
              <input
                type="url"
                required
